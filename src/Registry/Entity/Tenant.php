 ...
 * rest of what reads the control-plane database.
 *
 * The administration surface moved out to a package; this did not, because a
 * tenant's *own* request cannot start without it. `TenantResolver` turns a `Host`
 * header into one of these before anything else happens, and
 * `TenantConnectionParameters` reads this row to get the DSN and decrypt the
 * password it connects with. An instance serving customers with no registry in it
 * cannot answer the first question it is asked, which is why "move the control
 * plane out of the public instance" is not a sentence that can be carried out as
 * written (§3.1).
 *
 * So the rule for anything added here: this is the **read model of the registry**,
 * the part a customer's request needs. Anything that only an operator would ever
 * touch — provisioning, migration, usage, the module catalogue's write side —
 * belongs in the package, and deptrac will not let the arrow turn round.
 *
