...
- The invoice gains the `discount` kind as an ordinary kind. Nothing generates
  one there, a copy stays editable, and what to bill is decided on the
  invoice. Open, its own ticket (XIV-147): a partial invoice takes the whole
  discount. The discount line appears at first save; the totals follow live.
