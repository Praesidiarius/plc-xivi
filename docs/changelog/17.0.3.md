...
The release that learned to send. Mail is the through-line — a transport that
knows whose installation it is, templates written here instead of uploaded, a
send button on a record, and the invoice going out attached to it — and the same
week produced the two things that make an installation somebody else's: a store a
tenant installs modules from, on a permission axis of its own, and an invitation
that replaces a password read off somebody's screen.

Underneath it, a quieter half. `bin/ci` stopped trusting a stale container,
worktrees stopped sharing an image, a tenant can be thrown away and rebuilt from
one command, and the repository finally tells a coding agent how it is worked on.
None of that is a feature; all of it is why the features above landed in a day.
