# Xivi 17.0.6 — 2026-08-18

## Changed

- **The guard that keeps the administration surface optional is no longer in a
  file Symfony Flex regenerates** ([XIV-111],
  [§4.4](docs/architecture/deployment.md#one-of-the-three-seams-was-in-a-generated-file-xiv-111)).
...
- **An email can list a record's collection** — an order's lines, a contact's
  ...
  [§5.13.1](docs/architecture/data-model.md#5131-a-collection-in-an-email-body-xiv-62)).
...
- **A voucher module: codes a tenant hands out, with limits and three kinds of
  discount** ([XIV-103],
  [§5.19](docs/architecture/data-model.md#519-vouchers-and-a-counter-with-a-rule-in-it-xiv-103)).
...
- **A dashboard each person picks and arranges, over a default the installation
  sets** ([XIV-66],
  [§8.3.1](docs/architecture/identity-and-access.md#whose-dashboard-it-is-and-a-seam-a-module-can-reach-xiv-66)).
...
- **A customer-facing image that does not contain the administration surface**
  ([XIV-96], [§4.4](docs/architecture/deployment.md#44-two-images-what-a-customers-instance-is-built-without-xiv-96)).
...
- **A tenant can take what their module grew after they installed it**
  ([XIV-70], [§7.2.1](docs/architecture/open-questions.md#721-taking-what-a-module-grew-without-retro-fitting-it-xiv-70)).
...
- **An installation can declare which hostnames it answers to**, with
  ...
  ([XIV-93], [§4.3](docs/architecture/deployment.md#43-which-hostnames-this-installation-answers-to-xiv-93)).
...
- **The brief now says the control plane is not isolated by its hostname**
  ([§8.9](docs/architecture/identity-and-access.md#89-an-operator-is-not-a-tenant-user-xiv-57)).
...
- **A text field that is not numbered can be given a numbering pattern, from the
  metadata editor** ([XIV-91], [§5.10](docs/architecture/data-model.md#making-a-field-numbered-and-stopping-xiv-91)).
...
- **The tenant list shows the modules a customer actually has installed, and
  ...
  ([§8.11](docs/architecture/identity-and-access.md#what-a-tenant-actually-has-installed-and-where-that-disagrees-xiv-95)).
...
- **A module can have a price, and the operator sets it** ([XIV-101],
  [§6.5](docs/architecture/extensibility.md#65-a-module-can-have-a-price-and-the-operator-sets-it-xiv-101)).
...
- **The store shows what a module costs, and asking to buy one installs nothing**
  ([XIV-102],
  [§8.15](docs/architecture/identity-and-access.md#815-a-price-a-customer-can-see-and-an-ask-that-installs-nothing-xiv-102)).
...
- **An order with no lines can no longer be confirmed** ([XIV-110],
  [§5.8](docs/architecture/data-model.md#58-lifecycles-xiv-14)). A lifecycle could only
...
- **A field marked unique is now enforced by a unique index rather than by a
  ...
  ([XIV-109], [§7.2](docs/architecture/open-questions.md#7-open-design-questions)). It was a
...
- **A record form now counts its rows before it builds them** ([XIV-90]). The
  ...
  See [§5.1](docs/architecture/data-model.md#counting-the-rows-before-the-form-is-built-xiv-90),
...
- **Symfony's ExpressionLanguage is not adopted**, and the brief now says where
  ...
  [§5.8](docs/architecture/data-model.md#58-lifecycles-xiv-14)). Nothing changes for anybody
