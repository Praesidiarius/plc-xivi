...
numbers are not a sequence; no data migrates from v1). Solo project.

This brief is the authoritative context for design decisions: **a summary of
the engine and its decisions, not the story of how each landed.** The stories
...
four months and deptrac reported no violations for an empty configuration's
reason. **Plant a violation whenever a layer is added**; the standing
instruction and worked examples are in `deptrac.yaml`'s own header.

If external distribution is ever needed, use the `symfony/symfony` model
...
**Modules may need each other at runtime** (XIV-23): a module declares
`requires` (installing without one is refused, naming what to install first)
and `uses` (installing succeeds; the parts that depend on it are not offered).
Both hard would make "required" unreadable; the distinction is load-bearing all
over §5 and §6.
...
every tenant request begins in the control-plane database
(`TenantResolver`, `TenantConnectionParameters`), so an instance without it
cannot answer the first question it is asked. What is separable is the
**administration surface**. Two halves of one database:

- **The registry** (`App\Registry`, in `src/`): the tenant row, domains,
  encrypted credential, status, module catalogue. Read on every customer
...
- **The surface** (`Xivi\ControlPlane`, in `packages/control-plane`):
  ...
  usage collection, the introspector. Nothing on a customer's request path.

**The dependency rule runs opposite to the modules'**: the control plane may
depend on the application (provisioning drives tenancy and the tenant app's own
services) and on core; the application may never depend on the control plane
(deptrac fails the build; a serving instance can be built without the surface
compiled in, which §4.4 delivers). A module may depend on neither, and neither
on a module. `packages/xivi-mate` has the same above-the-application shape.

Deliberately kept in the application: `app.control_plane_host` (a deployment's
fact, feeding `app.system_hosts` and the trusted-host pattern, §4.3/§4.4), the
firewall seam (`security.firewalls` must be named by one source, so the
application splices the package's firewalls in behind a presence check, §4.4),
and `migrations/control/` (the namespace is recorded in
...
- **Core is not a public API and has no deprecation policy.** `Xivi\Core` may
  change in any release; every caller is in this repository and fixed in the
  same commit, and `bin/ci` green across every module is a stricter check than
...
What this does not close: whether a *vertical* can arrive without a deploy.
...
§4 to §8 live in `docs/architecture/`, one file per area, split out on
2026-08-19 (XIV-149) and **distilled to their decisions on 2026-08-20
(XIV-159)**: the issue-by-issue narratives were deleted in favour of the rules
...
`src/` and `packages/` is already comments; a paragraph here restating one is a
...
the version scheme (17 is a generation, not a semver major; the number moves on
release).

**§9.2 is in [architecture/decisions.md](architecture/decisions.md)** and keeps
its number; code cites it by number.
...
engine first, §5.4's own sentence (a type says which of its options are the
customer's to set) as applied to the two options a vertical is made of (§6.6),
and then somewhere in the control plane to live. Modules themselves are closed
(§3.2); the store may assume a curated catalogue.
...
per-tenant cache (§7.4 in a new costume), and file storage is half answered
(document templates live in the tenant database; attachments are many, large
and long-lived, and still want a real answer).
...
on XIV-61 too): the first test deployment goes to Germany because it is cheap
and nothing on it is real; **a Swiss datacenter is a hard precondition for
onboarding any paying customer**, and for the health wave it is not even a
discussion.
