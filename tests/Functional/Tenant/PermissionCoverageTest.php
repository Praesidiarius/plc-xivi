...
final class PermissionCoverageTest extends KernelTestCase
{
     ...
     *
     * **This used to have an escape hatch and no longer does.** XIV-80 shipped
     * the follow-up engine before any screen called it, so `follow_up_create` and
     * `follow_up_complete` were granted, enforced by a service, and named by no
     * route — a true statement rather than a bug, held in a documented
     * `ENFORCED_WITHOUT_A_ROUTE` list with the ticket that would empty it written
     * into each entry. XIV-82 emptied it by giving the record page real routes,
     * and the list went with it: a hatch nothing goes through is one that rots
     * open, and PHPStan says so out loud, since an empty list makes the check
     * guarding it an assertion that cannot fail.
     *
     * The next engine-first ticket in that shape should put it back rather than
     * weaken this, and should copy what made the old one safe: a mandatory reason
     * per entry, and a second test insisting the excused action is named
     * somewhere in `src/`.
     */
